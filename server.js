...
class YeelightBridgeServer {
    ...
    async addBulb(ip, token, name, bulbId) {
        try {
            ...
            let deviceModel = 'Yeelight';
            try {
                if (device.miioModel) {
                    deviceModel = device.miioModel;
                } else if (device.model) {
                    deviceModel = device.model;
                }
            } catch (e) {}
            
            const isMiotDevice = deviceModel.includes('xiaomi.light');
            let connectionTestPassed = false;
            let mihomeDevice = null;
            
            if (isMiotDevice) {
                console.log(`Initializing node-mihome for MIOT device: ${deviceModel}`);
                try {
                    mihome.miioProtocol.init();
                    mihome.miioProtocol.updateDevice(ip, {
                        id: bulbId,
                        token: token
                    });
                    mihomeDevice = {
                        send: async (method, params) => {
                            return await mihome.miioProtocol.send(ip, method, params, { token });
                        }
                    };
                    connectionTestPassed = true;
                    console.log(`✓ node-mihome initialized for ${deviceModel}`);
                } catch (mihomeError) {
                    console.log(`node-mihome init failed: ${mihomeError.message}, using miio directly`);
                }
            } else {
                try {
            ...
            if (!connectionTestPassed && !isMiotDevice) {
                console.log(`Warning: Connection test failed, but continuing for ${deviceModel}`);
            }
    ...
    async ensureMihomeDevice(bulb) {
        if (bulb.mihomeDevice) {
            return;
        }
        
        console.log(`Initializing node-mihome for MIOT device: ${bulb.model} at ${bulb.ip}`);
        try {
            mihome.miioProtocol.init();
            mihome.miioProtocol.updateDevice(bulb.ip, {
                id: bulb.id,
                token: bulb.token
            });
            bulb.mihomeDevice = {
                send: async (method, params) => {
                    return await mihome.miioProtocol.send(bulb.ip, method, params, { token: bulb.token });
                }
            };
            console.log(`✓ node-mihome initialized for ${bulb.model}`);
        } catch (initError) {
            throw new Error(`Failed to initialize node-mihome: ${initError.message}`);
        }
    }

    async setBulbColor(bulb, r, g, b, brightness = 100) {
        try {
            const isMiotDevice = bulb.model && bulb.model.includes('xiaomi.light');
            
            if (isMiotDevice) {
                await this.ensureMihomeDevice(bulb);
                
                const rgbValue = (r << 16) | (g << 8) | b;
                
                let deviceId = bulb.ip;
                try {
                    if (bulb.device && bulb.device.id) {
                        deviceId = bulb.device.id.replace('miio:', '');
                    }
                } catch (e) {}
                ...
                console.log(`Setting RGB via node-mihome MIOT: ${r}, ${g}, ${b} (${rgbValue}), did: ${deviceId}`);
                const result = await bulb.mihomeDevice.send('set_properties', params);
                
                if (!result || (Array.isArray(result) && result.length > 0 && result[0].code !== 0)) {
                    throw new Error(`MIOT set_properties failed: ${JSON.stringify(result)}`);
                }
                
                console.log(`✓ RGB color set successfully via MIOT`);
            } else {
    ...
    async setBulbBrightness(bulb, brightness) {
        try {
            ...
            if (isMiotDevice) {
                ...
                console.log(`Setting brightness via node-mihome MIOT: ${brightnessValue}%`);
                const result = await bulb.mihomeDevice.send('set_properties', params);
                ...
                console.log(`✓ Brightness set successfully via MIOT`);
            } else {
                await bulb.device.call('set_bright', [brightnessValue, 'smooth', 300]);
            }
    ...
    async setBulbPower(bulb, power) {
        try {
            ...
            if (isMiotDevice) {
                ...
                console.log(`Setting power via node-mihome MIOT: ${power ? 'on' : 'off'}`);
                const result = await bulb.mihomeDevice.send('set_properties', params);
                ...
                console.log(`✓ Power set successfully via MIOT`);
            } else {
